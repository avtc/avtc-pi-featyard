 ...
 *
 * Only PI_FF_FEATURE (child locates its state file on start) and PI_FF_STAGE
 * (root fork-mode derivation) are env-mirrored. PI_FF_REVIEW_LOOP and the
...
describe("env-sync", () => {
  ...
  describe("setActiveFeatureEnv", () => {
    it("sets PI_FF_FEATURE env var", () => {
      setActiveFeatureEnv("my-feature");
      expect(process.env.PI_FF_FEATURE).toBe("my-feature");
    });

    it("overwrites previous value", () => {
      setActiveFeatureEnv("first");
      setActiveFeatureEnv("second");
      expect(process.env.PI_FF_FEATURE).toBe("second");
    });
  });

  describe("clearActiveFeatureEnv", () => {
    it("deletes PI_FF_FEATURE env var", () => {
      process.env.PI_FF_FEATURE = "existing";
      clearActiveFeatureEnv();
  ...
  describe("clearFeatureEnvVars", () => {
    it("deletes stage", () => {
      process.env.PI_FF_STAGE = "design";
      clearFeatureEnvVars();
      expect(process.env.PI_FF_STAGE).toBeUndefined();
    });

    it("does not touch PI_FF_FEATURE (owned by clearActiveFeatureEnv)", () => {
      process.env.PI_FF_FEATURE = "keep-me";
      clearFeatureEnvVars();
      expect(process.env.PI_FF_FEATURE).toBe("keep-me");
    });
  });

  describe("syncEnvVarsFromState", () => {
    it("sets PI_FF_STAGE from currentPhase", () => {
      const handler = makeHandler({ phase: "design", slug: null });
      syncEnvVarsFromState(handler);
      expect(process.env.PI_FF_STAGE).toBe("design");
    });

    it("deletes PI_FF_STAGE when workflow state is null", () => {
      process.env.PI_FF_STAGE = "old";
      const handler = makeHandler({ phase: null, slug: null });
      syncEnvVarsFromState(handler);
      expect(process.env.PI_FF_STAGE).toBeUndefined();
    });

    it("sets PI_FF_STAGE regardless of whether a slug is active (stage is slug-independent)", () => {
      const handler = makeHandler({ phase: "review", slug: "my-feature" });
      syncEnvVarsFromState(handler);
      expect(process.env.PI_FF_STAGE).toBe("review");
    });

    it("sets PI_FF_STAGE even with no slug but phase set", () => {
