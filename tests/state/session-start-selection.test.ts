...
describe("session-start feature selection", () => {
  ...
  test("reason=startup always fresh starts — no feature list, no prompt", async () => {
    ...
    const state1 = createFeatureState("2026-01-01-first", "docs/featyard/designs/2026-01-01-first-design.md");
    saveFeatureState(state1, null);
    const state2 = createFeatureState("2026-02-01-second", "docs/featyard/designs/2026-02-01-second-design.md");
  ...
  test("reason=new with env var loads the bound feature without prompt", async () => {
    ...
    const state1 = createFeatureState("2026-01-01-old", "docs/featyard/designs/2026-01-01-old-design.md");
    saveFeatureState(state1, null);
    const state2 = createFeatureState("2026-02-01-new", "docs/featyard/designs/2026-02-01-new-design.md");
    saveFeatureState(state2, null);

    // Env var is set by the root session before creating new session
    process.env.PI_FY_FEATURE = "2026-02-01-new";
    // Workflow-initiated flag should also be set by /fy:next or task-tracker
  ...
  test("reason=new without env var fresh starts (no feature list, no prompt)", async () => {
    ...
    const state = createFeatureState("2026-05-01-active", "docs/featyard/designs/2026-05-01-active-design.md");
  ...
  test("subagent with PI_FY_FEATURE env loads that feature", async () => {
    enableSubagentMode();
    const fake = createFakePi();
    workflowMonitorExtension(fake.api as unknown as ExtensionAPI);

    const state = createFeatureState("2026-04-01-sub", "docs/featyard/designs/2026-04-01-sub-design.md");
    state.workflow.currentPhase = "implement";
    saveFeatureState(state, null);

    process.env.PI_FY_FEATURE = "2026-04-01-sub";
  ...
  test("subagent without env var does NOT load any feature", async () => {
    ...
    const state = createFeatureState("2026-04-01-subauto", "docs/featyard/designs/2026-04-01-subauto-design.md");
    saveFeatureState(state, null);
  ...
  test("reason=resume clears state when no session entries", async () => {
    ...
    process.env.PI_FY_FEATURE = "2026-05-10-old-feature";
  ...
  test("reason=new without workflow flag and env var set shows continue-or-reset prompt — continue", async () => {
    ...
    const state = createFeatureState("2026-05-10-active", "docs/featyard/designs/2026-05-10-active-design.md");
    state.workflow.currentPhase = "implement";
    saveFeatureState(state, null);
    process.env.PI_FY_FEATURE = "2026-05-10-active";
  ...
  test("reason=new without workflow flag and env var set — reset chosen", async () => {
    ...
    const state = createFeatureState("2026-05-10-reset-test", "docs/featyard/designs/2026-05-10-reset-test-design.md");
    state.workflow.currentPhase = "implement";
    saveFeatureState(state, null);
    process.env.PI_FY_FEATURE = "2026-05-10-reset-test";
  ...
  test("reason=new with workflow flag skips prompt and loads feature", async () => {
    ...
    const state = createFeatureState("2026-05-10-wf-new", "docs/featyard/designs/2026-05-10-wf-new-design.md");
    state.workflow.currentPhase = "implement";
    saveFeatureState(state, null);
    process.env.PI_FY_FEATURE = "2026-05-10-wf-new";
  ...
  test("reason=new without workflow flag and hasUI=false skips prompt", async () => {
    ...
    const state = createFeatureState("2026-05-10-sub-new", "docs/featyard/designs/2026-05-10-sub-new-design.md");
    saveFeatureState(state, null);
    process.env.PI_FY_FEATURE = "2026-05-10-sub-new";
