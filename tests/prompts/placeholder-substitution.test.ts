...
// mkdirSync(".ff/reviews/<date>") through the production fs, which must land in the temp, never
// the real repo's.ff junction / external store.
beforeEach(() => {
...
describe("substitutePlaceholders", () => {
  ...
  it("replaces {{PI_FF_ARCHITECTURE_PRINCIPLES}} with the canonical principles block", () => {
    const result = substitutePlaceholders("## Architecture\n{{PI_FF_ARCHITECTURE_PRINCIPLES}}\nEnd.", {});
    expect(result).not.toContain("{{PI_FF_ARCHITECTURE_PRINCIPLES}}");
  ...
  it("replaces {{PI_FF_COVERAGE_REVIEW_PROCESS}} with the coverage-first review process block", () => {
    const result = substitutePlaceholders("## Process\n{{PI_FF_COVERAGE_REVIEW_PROCESS}}\nEnd.", {});
    expect(result).not.toContain("{{PI_FF_COVERAGE_REVIEW_PROCESS}}");
  ...
  it("replaces {{PI_FF_DOC_COVERAGE_PROCESS}} with the doc coverage-first process block", () => {
    const result = substitutePlaceholders("## Process\n{{PI_FF_DOC_COVERAGE_PROCESS}}\nEnd.", {});
    expect(result).not.toContain("{{PI_FF_DOC_COVERAGE_PROCESS}}");
    expect(result).toContain("## Process\n");
    expect(result).not.toContain("## Process — Coverage-First Review");
  ...
  it("replaces {{PI_FF_ADDITIONAL_AREAS_OF_ATTENTION}} with the concerns checklist (no heading)", () => {
    const result = substitutePlaceholders("{{PI_FF_ADDITIONAL_AREAS_OF_ATTENTION}}", {});
    expect(result).not.toContain("{{PI_FF_ADDITIONAL_AREAS_OF_ATTENTION}}");
  ...
  it("replaces {{PI_FF_WORTH_NOTES_PATH}} with slug-based path when slug provided", () => {
    const result = substitutePlaceholders("Save to {{PI_FF_WORTH_NOTES_PATH}}.", {
      slug: "2026-06-23-foo",
    });
    expect(result).toBe("Save to .ff/reviews/2026-06-23-foo/2026-06-23-foo-worth-notes.md.");
    expect(result).not.toContain("{{PI_FF_WORTH_NOTES_PATH}}");
  });

  it("replaces {{PI_FF_WORTH_NOTES_PATH}} with date-based fallback when no slug", () => {
    const result = substitutePlaceholders("Save to {{PI_FF_WORTH_NOTES_PATH}}.", {});
    // No slug -> date-based accumulated file (manual ff-implement invocation)
    expect(result).toMatch(/\.ff\/reviews\/\d{4}-\d{2}-\d{2}\/\d{4}-\d{2}-\d{2}-worth-notes\.md\./);
    expect(result).not.toContain("{{PI_FF_WORTH_NOTES_PATH}}");
    expect(result).not.toContain("(not available)");
  });

  it("replaces {{PI_FF_IMPLEMENTER_GUIDANCE}} with the full guidance (inner placeholders inlined)", () => {
    const result = substitutePlaceholders("{{PI_FF_IMPLEMENTER_GUIDANCE}}", {});
    expect(result).not.toContain("{{PI_FF_IMPLEMENTER_GUIDANCE}}");
  ...
  it("replaces {{PI_FF_REVIEWER_SKIP}} with skip context", () => {
    const result = substitutePlaceholders("Skip: {{PI_FF_REVIEWER_SKIP}} end", {});
    expect(result).not.toContain("{{PI_FF_REVIEWER_SKIP}}");
    expect(result).toContain("threshold");
  });

  it("handles both placeholders in same text", () => {
    const result = substitutePlaceholders("{{PI_FF_FEATURE_SLUG}} and {{PI_FF_REVIEWER_SKIP}}", {});
    expect(result).not.toContain("{{PI_FF_");
  });

  it("replaces {{PI_FF_REPORT_FILE}} with a date-based fallback when no slug or agentName provided", () => {
    const result = substitutePlaceholders("Write only to `{{PI_FF_REPORT_FILE}}`.", {});
    expect(result).toMatch(/\.ff\/reviews\/\d{4}-\d{2}-\d{2}\/\d{4}-\d{2}-\d{2}-(?:report|review)(-\S+)?-\d+\.md/);
    expect(result).not.toContain("{{PI_FF_REPORT_FILE}}");
  });

  it("replaces {{PI_FF_REPORT_FILE}} with slug+agentName derived path", () => {
    const result = substitutePlaceholders("Write only to `{{PI_FF_REPORT_FILE}}`.", {
    ...
    expect(result).toBe(
      "Write only to `.ff/reviews/2026-06-01-codebase-refactoring/2026-06-01-codebase-refactoring-quality-2.md`.",
    );
    expect(result).not.toContain("{{PI_FF_REPORT_FILE}}");
  });

  it("replaces {{PI_FF_REPORT_FILE}} with slug+agentName path without loop number", () => {
    const result = substitutePlaceholders("Write only to `{{PI_FF_REPORT_FILE}}`.", {
      slug: "2026-06-01-codebase-refactoring",
      agentName: "quality",
    });
    expect(result).toBe(
      "Write only to `.ff/reviews/2026-06-01-codebase-refactoring/2026-06-01-codebase-refactoring-quality.md`.",
    );
  });

  it("replaces {{PI_FF_REPORT_FILE}} with slug+agentName+taskName path for per-task review", () => {
    const result = substitutePlaceholders("Write only to `{{PI_FF_REPORT_FILE}}`.", {
      slug: "2026-06-01-nonexistent-feature",
      agentName: "ff-general-reviewer",
      taskName: "3. Wire the login form",
      loopIndex: 0,
    });
    expect(result).toBe(
      "Write only to `.ff/reviews/2026-06-01-nonexistent-feature/2026-06-01-nonexistent-feature-task-3-wire-the-login-form-ff-general-reviewer-0.md`.",
    );
  });

  it("replaces {{PI_FF_REPORT_FILE}} with slug+agentName+taskName path without loop number", () => {
    const result = substitutePlaceholders("Write only to `{{PI_FF_REPORT_FILE}}`.", {
      slug: "2026-06-01-nonexistent-feature",
      agentName: "ff-general-reviewer",
    ...
    expect(result).toBe(
      "Write only to `.ff/reviews/2026-06-01-nonexistent-feature/2026-06-01-nonexistent-feature-task-5-add-validator-ff-general-reviewer.md`.",
    );
  });

  it("replaces {{PI_FF_REPORT_FILE}} with loopIndex 0 (first review loop)", () => {
    const result = substitutePlaceholders("Write only to `{{PI_FF_REPORT_FILE}}`.", {
      slug: "2026-06-01-nonexistent-feature",
      agentName: "ff-quality-reviewer",
      loopIndex: 0,
    });
    expect(result).toBe(
      "Write only to `.ff/reviews/2026-06-01-nonexistent-feature/2026-06-01-nonexistent-feature-ff-quality-reviewer-0.md`.",
    );
  });

  it("replaces {{PI_FF_REVIEWER_SKIP}} with actual emptyLoops data", () => {
    const emptyLoops = { "ff-quality-reviewer": 2, "ff-testing-reviewer": 0 };
    const result = substitutePlaceholders("Skip: {{PI_FF_REVIEWER_SKIP}} end", { emptyLoops });
    expect(result).toContain("ff-quality-reviewer");
    expect(result).toContain("2");
    expect(result).toContain("ff-testing-reviewer");
...
describe("PI_FF_DESIGN_REPORT_FILE placeholder", () => {
  beforeEach(() => {
    resetSettingsState();
  });

  it("replaces with slug-based path when slug provided", () => {
    const result = substitutePlaceholders("Write to {{PI_FF_DESIGN_REPORT_FILE}}.", { slug: "my-feature" });
    expect(result).toMatch(/\.ff\/reviews\/my-feature\/my-feature-design-review\.md/);
    expect(result).not.toContain("{{PI_FF_DESIGN_REPORT_FILE}}");
  });

  it("falls back to a date-based path when no slug provided", () => {
    const result = substitutePlaceholders("Write to {{PI_FF_DESIGN_REPORT_FILE}}.", {});
    expect(result).toMatch(/\.ff\/reviews\/\d{4}-\d{2}-\d{2}\/\d{4}-\d{2}-\d{2}-design-review-\S*-\d+\.md/);
    expect(result).not.toContain("{{PI_FF_DESIGN_REPORT_FILE}}");
  });
});

describe("PI_FF_PLAN_REPORT_FILE placeholder", () => {
  beforeEach(() => {
    resetSettingsState();
  });

  it("replaces with slug-based path when slug provided", () => {
    const result = substitutePlaceholders("Write to {{PI_FF_PLAN_REPORT_FILE}}.", { slug: "my-feature" });
    expect(result).toMatch(/\.ff\/reviews\/my-feature\/my-feature-plan-review\.md/);
    expect(result).not.toContain("{{PI_FF_PLAN_REPORT_FILE}}");
  });

  it("falls back to a date-based path when no slug provided", () => {
    const result = substitutePlaceholders("Write to {{PI_FF_PLAN_REPORT_FILE}}.", {});
    expect(result).toMatch(/\.ff\/reviews\/\d{4}-\d{2}-\d{2}\/\d{4}-\d{2}-\d{2}-plan-review-\S*-\d+\.md/);
    expect(result).not.toContain("{{PI_FF_PLAN_REPORT_FILE}}");
  });
});
...
describe("both report file placeholders together", () => {
  ...
  it("both placeholders in same text produce consistent paths", () => {
    const result = substitutePlaceholders("Design: {{PI_FF_DESIGN_REPORT_FILE}} and Plan: {{PI_FF_PLAN_REPORT_FILE}}", {
      slug: "my-feature",
    });
    expect(result).not.toContain("{{PI_FF_");

    // Verify slug subfolder is present in both paths
    expect(result).toContain(".ff/reviews/my-feature/");
...
describe("PI_FF_DESIGN_REPORT_FILE with loopIndex", () => {
  beforeEach(() => {
    resetSettingsState();
  });
  ...
  it("appends loop index suffix when loopIndex provided", () => {
    ...
    expect(result).toMatch(/\.ff\/reviews\/my-feature\/my-feature-design-review-3\.md/);
    expect(result).not.toContain("{{PI_FF_DESIGN_REPORT_FILE}}");
  });
  ...
  it("appends loop index 0 suffix", () => {
    ...
    expect(result).toMatch(/\.ff\/reviews\/my-feature\/my-feature-design-review-0\.md/);
  });
});

describe("PI_FF_PLAN_REPORT_FILE with loopIndex", () => {
  beforeEach(() => {
    resetSettingsState();
  });

  it("appends loop index suffix when loopIndex provided", () => {
    const result = substitutePlaceholders("Write to {{PI_FF_PLAN_REPORT_FILE}}.", { slug: "my-feature", loopIndex: 2 });
    expect(result).toMatch(/\.ff\/reviews\/my-feature\/my-feature-plan-review-2\.md/);
    expect(result).not.toContain("{{PI_FF_PLAN_REPORT_FILE}}");
  });
});

describe("PI_FF_REVIEW_REPORT_FILE placeholder", () => {
  beforeEach(() => {
    resetSettingsState();
  });

  it("replaces with slug-based path when slug provided", () => {
    const result = substitutePlaceholders("Write to {{PI_FF_REVIEW_REPORT_FILE}}.", { slug: "my-feature" });
    expect(result).toMatch(/\.ff\/reviews\/my-feature\/my-feature-review\.md/);
    expect(result).not.toContain("{{PI_FF_REVIEW_REPORT_FILE}}");
  });

  it("falls back to a date-based path when no slug provided", () => {
    const result = substitutePlaceholders("Write to {{PI_FF_REVIEW_REPORT_FILE}}.", {});
    expect(result).toMatch(/\.ff\/reviews\/\d{4}-\d{2}-\d{2}\/\d{4}-\d{2}-\d{2}-review-\S*-\d+\.md/);
    expect(result).not.toContain("{{PI_FF_REVIEW_REPORT_FILE}}");
  });
  ...
  it("appends loop index suffix when loopIndex provided", () => {
    ...
    expect(result).toMatch(/\.ff\/reviews\/my-feature\/my-feature-review-2\.md/);
    expect(result).not.toContain("{{PI_FF_REVIEW_REPORT_FILE}}");
  });
  ...
  it("appends loop index 0 suffix", () => {
    ...
    expect(result).toMatch(/\.ff\/reviews\/my-feature\/my-feature-review-0\.md/);
  });
});
...
describe("buildReportFilePath", () => {
  it("builds design review path with loop number", () => {
    ...
    expect(path).toBe(".ff/reviews/my-feature/my-feature-design-review-2.md");
  });

  it("builds plan review path with loop number", () => {
    const path = buildReportFilePath("my-feature", "plan-review", 0, mockFs(null, null));
    expect(path).toBe(".ff/reviews/my-feature/my-feature-plan-review-0.md");
  });
  ...
  it("builds path without loop number when undefined and no files exist", () => {
    ...
    expect(path).toBe(".ff/reviews/my-feature/my-feature-design-review.md");
  });
  ...
  it("picks next available number when counter=0 but files exist on disk", () => {
    ...
    expect(filePath).toBe(".ff/reviews/my-feature/my-feature-design-review-2.md");
  });
  ...
  it("uses counter when counter > highest file on disk", () => {
    ...
    expect(filePath).toBe(".ff/reviews/my-feature/my-feature-design-review-5.md");
  });
  ...
  it("picks next number when loopNumber is undefined but files exist", () => {
    ...
    expect(filePath).toBe(".ff/reviews/my-feature/my-feature-design-review-4.md");
  });
  ...
  it("ignores files with different prefix when scanning", () => {
    ...
    expect(filePath).toBe(".ff/reviews/my-feature/my-feature-design-review-0.md");
  });
  ...
  it("handles manual review after extension loop finished (counter reset, files exist)", () => {
    ...
    expect(filePath).toBe(".ff/reviews/my-feature/my-feature-review-3.md");
  });
  ...
  it("handles slug with special regex characters", () => {
    ...
    expect(filePath).toBe(".ff/reviews/my.feature/my.feature-review-1.md");
  });
});

describe("PI_FF_DESIGN_DOC_PATH placeholder", () => {
  beforeEach(() => {
    resetSettingsState();
  });
  ...
  it("replaces with slug-based exact path when slug provided (committed mode)", () => {
    ...
    const result = substitutePlaceholders("Save to {{PI_FF_DESIGN_DOC_PATH}}.", {
      slug: "2026-05-22-my-feature",
    });
    expect(result).toBe("Save to docs/ff/designs/2026-05-22-my-feature-design.md.");
  ...
  it("replaces with the local path when designDocStorage = local (default)", () => {
    ...
    expect(result).toBe("Save to .ff/designs/2026-05-22-my-feature-design.md.");
  });

  it("replaces with template hint when no slug provided (local mode default)", () => {
    const result = substitutePlaceholders("Save to {{PI_FF_DESIGN_DOC_PATH}}.", {});
    expect(result).toContain(".ff/designs/YYYY-MM-DD-<topic>-design.md");
    expect(result).not.toContain("{{PI_FF_DESIGN_DOC_PATH}}");
  });
});

describe("PI_FF_RESEARCH_DIR placeholder", () => {
  beforeEach(() => {
    resetSettingsState();
  });

  it("replaces with absolute path when slug provided", () => {
    const result = substitutePlaceholders("Write to {{PI_FF_RESEARCH_DIR}}.", {
      slug: "2026-06-01-codebase-refactoring",
    });
    const expected = path.resolve(process.cwd(), ".ff", "research", "2026-06-01-codebase-refactoring");
    expect(result).toBe(`Write to ${expected}.`);
    expect(result).not.toContain("{{PI_FF_RESEARCH_DIR}}");
  });
  ...
  it("replaces with date-wrapped fallback path when no slug provided", () => {
    ...
    expect(result).toMatch(/Write to (.*)\.ff[\\/]research[\\/](\d{4}-\d{2}-\d{2})[\\/]\2-<topic>[\\/]?\.$/);
    expect(path.isAbsolute(result.replace(/^Write to |\.$/g, ""))).toBe(true);
    expect(result).not.toContain("{{PI_FF_RESEARCH_DIR}}");
  });

  it("replaces multiple occurrences", () => {
    const result = substitutePlaceholders("Input: {{PI_FF_RESEARCH_DIR}}\nOutput: {{PI_FF_RESEARCH_DIR}}", {
      slug: "my-feature",
    });
    const expected = path.resolve(process.cwd(), ".ff", "research", "my-feature");
    expect(result).toBe(`Input: ${expected}\nOutput: ${expected}`);
  });

  it("produces absolute path (not relative)", () => {
    const result = substitutePlaceholders("{{PI_FF_RESEARCH_DIR}}", {
      slug: "test-slug",
    });
...
describe("PI_FF_PLAN_DOC_PATH placeholder", () => {
  beforeEach(() => {
    resetSettingsState();
  });

  it("replaces with slug-based exact path when slug provided", () => {
    const result = substitutePlaceholders("Save to {{PI_FF_PLAN_DOC_PATH}}.", {
      slug: "2026-05-22-my-feature",
    });
    expect(result).toBe("Save to .ff/task-plans/2026-05-22-my-feature-task-plan.md.");
  ...
  it("replaces with date-wrapped fallback path when no slug provided", () => {
    ...
    expect(result).toMatch(
      /^Save to \.ff[\\/]task-plans[\\/](\d{4}-\d{2}-\d{2})[\\/]\1-<feature-name>-task-plan\.md\.$/,
    );
    expect(result).not.toContain("{{PI_FF_PLAN_DOC_PATH}}");
  });

  it("replaces multiple occurrences (slug path)", () => {
    const result = substitutePlaceholders("Plan A: {{PI_FF_PLAN_DOC_PATH}}\nPlan B: {{PI_FF_PLAN_DOC_PATH}}", {
      slug: "my-feature",
    });
    expect(result).toBe(
      "Plan A: .ff/task-plans/my-feature-task-plan.md\nPlan B: .ff/task-plans/my-feature-task-plan.md",
    );
  });
});

describe("PI_FF_DESIGN_RELATIVE_DIR placeholder", () => {
  beforeEach(() => {
    resetSettingsState();
  });
  ...
  it("resolves to the local design-doc directory by default (designDocStorage = local)", () => {
    ...
    // directory selected by the designDocStorage setting — 'local' (.ff/designs) by default.
    const result = substitutePlaceholders("Write design docs to {{PI_FF_DESIGN_RELATIVE_DIR}}/.", {});
    expect(result).toBe("Write design docs to .ff/designs/.");
    expect(result).not.toContain("{{PI_FF_DESIGN_RELATIVE_DIR}}");
  });

  it("resolves to the in-repo directory when designDocStorage = committed", () => {
    setSetting("designDocStorage", "committed");
    const result = substitutePlaceholders("{{PI_FF_DESIGN_RELATIVE_DIR}}", {});
    expect(result).toBe("docs/ff/designs");
  });

  it("is independent of slug/date (constant dir per mode, not a per-feature path)", () => {
    const noSlug = substitutePlaceholders("{{PI_FF_DESIGN_RELATIVE_DIR}}", {});
    const withSlug = substitutePlaceholders("{{PI_FF_DESIGN_RELATIVE_DIR}}", {
      slug: "2026-06-29-x",
    });
    expect(noSlug).toBe(".ff/designs");
    expect(withSlug).toBe(".ff/designs");
  });
});

describe("PI_FF_DESIGN_HANDOFF placeholder", () => {
  beforeEach(() => {
    resetSettingsState();
  });

  it("instructs to skip the commit when designDocStorage = local (default)", () => {
    const result = substitutePlaceholders("- {{PI_FF_DESIGN_HANDOFF}}", {});
    expect(result).toBe(
      "- The design file is gitignored under `.ff/` (do not commit). Signal the phase complete with the `phase_ready` tool. End your turn.",
    );
    expect(result).not.toContain("{{PI_FF_DESIGN_HANDOFF}}");
...
describe("PI_FF_REVIEWER_DISPATCH", () => {
  ...
  it("injects comprehensive dispatch when featureReviewMode is comprehensive", () => {
    ...
    expect(result).toContain("ff-quality-reviewer");
    expect(result).toContain("ff-testing-reviewer");
    expect(result).toContain("ff-security-reviewer");
    expect(result).toContain("ff-performance-reviewer");
    expect(result).toContain("ff-guidelines-reviewer");
    expect(result).toContain("ff-requirements-reviewer");
    expect(result).toContain("Dispatch ALL 6");
  });

  it("pre-resolves PI_FF_REVIEWER_SKIP before injection", () => {
    setSetting("featureReviewMode", "general");
    const result = substitutePlaceholders("Step 2:\n{{PI_FF_REVIEWER_DISPATCH}}", {});
    expect(result).not.toContain("{{PI_FF_REVIEWER_SKIP}}");
  });
...
describe("PI_FF_VERIFY_PHASES", () => {
  it("resolves to content block when phase is included", () => {
    setSetting("verifyPhases", "plan+implement+verify");
    const input = "Before\n{{PI_FF_VERIFY_PHASES:verify}}\nAfter";
    const result = substitutePlaceholders(input, {});
    expect(result).toContain("Spawn ff-feature-verifier");
    expect(result).toContain("Fallback");
    expect(result).toContain("Before");
    expect(result).toContain("After");
    expect(result).not.toContain("{{PI_FF_VERIFY_ITERATIONS}}");
  });
  ...
  it("defaults to 3 iterations when maxVerifyRounds is unset", () => {
    ...
    const result = substitutePlaceholders("{{PI_FF_VERIFY_PHASES:verify}}", {});
  ...
  it("resolves plan phase correctly", () => {
    ...
    const result = substitutePlaceholders("{{PI_FF_VERIFY_PHASES:plan}}", {});
    expect(result).toContain("Spawn `ff-plan-verifier`");
  });

  it("unrecognized phase name resolves to empty string", () => {
    setSetting("verifyPhases", "verify");
    const result = substitutePlaceholders("{{PI_FF_VERIFY_PHASES:unknown}}", {});
    expect(result).not.toContain("Spawn");
  });
...
describe("PI_FF_RESEARCHER_DELEGATION", () => {
  ...
  it("defaults to delegation section when nestedResearchers is not specified", () => {
    resetSettingsState();
    const result = substitutePlaceholders("{{PI_FF_RESEARCHER_DELEGATION}}", {});
    expect(result).toContain("## Delegation");
...
describe("malformed placeholder syntax", () => {
  ...
  it("leaves unclosed {{PI_FF_ prefix unchanged", () => {
    const text = "Before {{PI_FF_FEATURE_SLUG after";
    const result = substitutePlaceholders(text, {});
    expect(result).toBe(text);
  });

  it("leaves partial placeholder name unchanged", () => {
    const text = "{{PI_FF_REVIEW}} is not a real placeholder";
    const result = substitutePlaceholders(text, {});
    expect(result).toBe(text);
  });
  ...
  it("handles triple-braced placeholder gracefully", () => {
    const text = "{{{PI_FF_FEATURE_SLUG}}} extra brace";
    // The function replaces {{PI_FF_FEATURE_SLUG}} leaving the outer { and }
    const result = substitutePlaceholders(text, {});
    expect(result).toContain("YYYY-MM-DD");
    expect(result).not.toContain("{{PI_FF_FEATURE_SLUG}}");
  });

  it("handles adjacent placeholders without separator", () => {
    setSetting("featureReviewMode", "general");
    const text = "{{PI_FF_FEATURE_SLUG}}{{PI_FF_REVIEWER_DISPATCH}}";
    const result = substitutePlaceholders(text, {});
    expect(result).not.toContain("{{PI_FF_FEATURE_SLUG}}");
    expect(result).not.toContain("{{PI_FF_REVIEWER_DISPATCH}}");
  });

  it("does not replace PI_FF_ prefix without braces", () => {
    const text = "PI_FF_FEATURE_SLUG is just text";
    const result = substitutePlaceholders(text, {});
    expect(result).toBe(text);
  });
});
...
describe("buildFallbackReportPath", () => {
  it("returns N=1 when directory does not exist", () => {
    const path = buildFallbackReportPath("2026-06-05", "my-topic", "ff-general-reviewer", mockFs(false, null));
    expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-my-topic-ff-general-reviewer-1.md");
  });

  it("returns N=1 when directory is empty", () => {
    const path = buildFallbackReportPath("2026-06-05", "my-topic", "ff-general-reviewer", mockFs(true, []));
    expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-my-topic-ff-general-reviewer-1.md");
  });
  ...
  it("returns N=3 when existing files end in -1 and -2", () => {
    const path = buildFallbackReportPath(
      ...
      "my-topic",
      "ff-general-reviewer",
      mockFs(true, ["2026-06-05-my-topic-ff-general-reviewer-1.md", "2026-06-05-my-topic-ff-general-reviewer-2.md"]),
    );
    expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-my-topic-ff-general-reviewer-3.md");
  });
  ...
  it("only matches files for the same topic+agent combo", () => {
    const path = buildFallbackReportPath(
      ...
      "ff-general-reviewer",
      mockFs(true, [
        "2026-06-05-other-topic-ff-general-reviewer-5.md",
        "2026-06-05-my-topic-other-agent-1.md",
        "2026-06-05-my-topic-ff-general-reviewer-2.md",
      ]),
    );
    expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-my-topic-ff-general-reviewer-3.md");
  });
  ...
  it("skips non-matching files in the directory", () => {
    const path = buildFallbackReportPath(
      ...
      "ff-general-reviewer",
      mockFs(true, [
        "2026-06-05-my-topic-ff-general-reviewer-1.md",
        "some-other-file.md",
        "2026-06-04-my-topic-ff-general-reviewer-99.md",
        "2026-06-05-my-topic-known-issues.md",
      ]),
    );
    expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-my-topic-ff-general-reviewer-2.md");
  });
  ...
  it("handles topic and agentName with special characters", () => {
    const path = buildFallbackReportPath(
      ...
      "task-1-verification",
      "ff-task-verifier",
      mockFs(true, ["2026-06-05-task-1-verification-ff-task-verifier-1.md"]),
    );
    expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-task-1-verification-ff-task-verifier-2.md");
  });
  ...
  it("handles undefined agentName — omits agent suffix from filename", () => {
    ...
    expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-my-topic-1.md");
  });
  ...
  it("handles undefined agentName — matches existing files without agent suffix", () => {
    ...
    expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-my-topic-3.md");
  });
  ...
  describe("sanitization of LLM-generated input", () => {
    it("strips path traversal from topic", () => {
      ...
      expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-etc-passwd-1.md");
    });
    ...
    it("replaces path separators with hyphens", () => {
      ...
      expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-some-path-here-1.md");
    });
    ...
    it("strips special characters from agentName", () => {
      ...
      expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-review-agentwithspacesspecialchars-1.md");
    });
    ...
    it("falls back to 'review' when topic sanitizes to empty", () => {
      ...
      expect(path).toBe(".ff/reviews/2026-06-05/2026-06-05-review-1.md");
    });
  });

  describe("{{PI_FF_KNOWN_ISSUES_PATH}}", () => {
    it("replaces with per-feature known-issues path when slug is provided (no task)", () => {
      const result = substitutePlaceholders("Issues: {{PI_FF_KNOWN_ISSUES_PATH}}", { slug: "my-feature" });
      expect(result).toBe("Issues: .ff/reviews/my-feature/my-feature-known-issues.md");
    });

    it("replaces with per-task known-issues path when taskName is provided", () => {
      const result = substitutePlaceholders("Issues: {{PI_FF_KNOWN_ISSUES_PATH}}", {
        slug: "my-feature",
        taskName: "3. Wire the login form",
      });
      expect(result).toBe("Issues: .ff/reviews/my-feature/my-feature-task-3-wire-the-login-form-known-issues.md");
    });
    ...
    it("replaces with design known-issues path when phase is design", () => {
      ...
      expect(result).toBe("Issues: .ff/reviews/my-feature/my-feature-design-known-issues.md");
    });
    ...
    it("replaces with plan known-issues path when phase is plan", () => {
      ...
      expect(result).toBe("Issues: .ff/reviews/my-feature/my-feature-plan-known-issues.md");
    });
    ...
    it("replaces with date-based fallback path when slug is not provided", () => {
      ...
      expect(result).toMatch(/^Issues: \.ff\/reviews\/\d{4}-\d{2}-\d{2}\/\d{4}-\d{2}-\d{2}-review-known-issues\.md$/);
    });

    it("replaces with date-based fallback path when slug is empty string", () => {
      const result = substitutePlaceholders("Issues: {{PI_FF_KNOWN_ISSUES_PATH}}", { slug: "" });
      expect(result).toMatch(/^Issues: \.ff\/reviews\/\d{4}-\d{2}-\d{2}\/\d{4}-\d{2}-\d{2}-review-known-issues\.md$/);
    });

    it("date fallback is phase-scoped (design/plan) and per-task when taskName is set", () => {
      expect(substitutePlaceholders("{{PI_FF_KNOWN_ISSUES_PATH}}", { phase: "design" })).toMatch(
        /\d{4}-\d{2}-\d{2}-design-known-issues\.md$/,
      );
      expect(substitutePlaceholders("{{PI_FF_KNOWN_ISSUES_PATH}}", { phase: "plan" })).toMatch(
        /\d{4}-\d{2}-\d{2}-plan-known-issues\.md$/,
      );
      expect(substitutePlaceholders("{{PI_FF_KNOWN_ISSUES_PATH}}", { taskName: "7. Final task" })).toMatch(
        /\d{4}-\d{2}-\d{2}-task-7-final-task-known-issues\.md$/,
      );
    });

    it("replaces multiple occurrences", () => {
      const result = substitutePlaceholders("See {{PI_FF_KNOWN_ISSUES_PATH}} and {{PI_FF_KNOWN_ISSUES_PATH}}", {
        slug: "test-feature",
      });
      expect(result).toBe(
        "See .ff/reviews/test-feature/test-feature-known-issues.md and .ff/reviews/test-feature/test-feature-known-issues.md",
      );
    });
  ...
  describe("{{PI_FF_CURRENT_TASK}}", () => {
    it("replaces with the task designation when provided", () => {
      const result = substitutePlaceholders("Task: {{PI_FF_CURRENT_TASK}}", { taskName: "3. Wire the login form" });
      expect(result).toBe("Task: 3. Wire the login form");
    });

    it("replaces with not available when taskName is not provided", () => {
      const result = substitutePlaceholders("Task: {{PI_FF_CURRENT_TASK}}", {});
      expect(result).toBe("Task: (not available)");
    });

    it("replaces with not available when taskName is undefined", () => {
      const result = substitutePlaceholders("Task: {{PI_FF_CURRENT_TASK}}", { taskName: undefined });
      expect(result).toBe("Task: (not available)");
    });
  ...
  describe("{{PI_FF_BASE_COMMIT_SHA}}", () => {
    it("resolves from baseCommitSha option", () => {
      const result = substitutePlaceholders("Base: {{PI_FF_BASE_COMMIT_SHA}}", {
    ...
    it("resolves to (not available) when baseCommitSha is undefined", () => {
      const result = substitutePlaceholders("Base: {{PI_FF_BASE_COMMIT_SHA}}", { baseCommitSha: undefined });
      expect(result).toBe("Base: (not available)");
    });
...
describe("substitutePlaceholders unresolved placeholder warning", () => {
  ...
  it("returns text unchanged when unknown PI_FF_ placeholder remains after substitution", () => {
    const text = "Some text {{PI_FF_UNKNOWN_FUTURE}} more text";
    const result = substitutePlaceholders(text, {});
    // Unknown placeholder should remain in output
    expect(result).toContain("{{PI_FF_UNKNOWN_FUTURE}}");
  });

  it("resolves all known placeholders without leaving any PI_FF_ markers", () => {
    setSetting("researcherMinInstances", 2);
    setSetting("researcherMaxInstances", 5);
    const text = "{{PI_FF_FEATURE_SLUG}} {{PI_FF_RESEARCHER_MIN}} {{PI_FF_RESEARCHER_MAX}}";
    const result = substitutePlaceholders(text, {});
    expect(result).not.toContain("{{PI_FF_");
  });
});

describe("PI_FF_REVIEW_METHOD placeholder", () => {
  beforeEach(() => {
    resetSettingsState();
  });
  afterEach(() => {
  ...
  it("resolves to design review method when phase is design", () => {
    const result = substitutePlaceholders("Method: {{PI_FF_REVIEW_METHOD}}", {
  ...
  it("resolves to empty when no review phase", () => {
    const result = substitutePlaceholders("Method: {{PI_FF_REVIEW_METHOD}}", { phase: "implement" });
...
describe("PI_FF_REVIEW_LOOP_NUMBER placeholder", () => {
  beforeEach(() => {
    resetSettingsState();
  });
  afterEach(() => {
  ...
  it("resolves to the display loop number for design/plan phases", () => {
    ...
    const design = substitutePlaceholders("{{PI_FF_REVIEW_LOOP_NUMBER}}", {
      slug: "2026-06-23-my-feat",
      phase: "design",
      loopIndex: 2,
    });
    ...
    const plan = substitutePlaceholders("{{PI_FF_REVIEW_LOOP_NUMBER}}", {
      slug: "2026-06-23-my-feat",
      phase: "plan",
      loopIndex: 1,
    });
  ...
  it("resolves to empty when not in a review phase", () => {
    const implement = substitutePlaceholders("{{PI_FF_REVIEW_LOOP_NUMBER}}", {
      slug: "x",
      phase: "implement",
      loopIndex: 2,
    });
  ...
  it("defaults to display loop #0 when loopIndex is omitted", () => {
    ...
    const result = substitutePlaceholders("{{PI_FF_REVIEW_LOOP_NUMBER}}", {
      slug: "2026-06-23-my-feat",
      phase: "design",
    });
