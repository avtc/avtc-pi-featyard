...
describe("PI_FF_FEATURE_SLUG placeholder", () => {
  beforeEach(resetSettingsState);
  afterEach(resetSettingsState);

  it("replaces with slug when provided", () => {
    const result = substitutePlaceholders("Path: {{PI_FF_FEATURE_SLUG}}", {
...
describe("PI_FF_RESEARCHER_MIN placeholder", () => {
  beforeEach(resetSettingsState);
  afterEach(resetSettingsState);

  it("replaces with the numeric value of researcherMinInstances", () => {
    setSetting("researcherMinInstances", 3);
    const result = substitutePlaceholders("Spawn {{PI_FF_RESEARCHER_MIN}} researchers", {});
    expect(result).toBe("Spawn 3 researchers");
    expect(result).not.toContain("{{PI_FF_RESEARCHER_MIN}}");
  });

  it("replaces with 0 when researcherMinInstances is 0", () => {
    setSetting("researcherMinInstances", 0);
    const result = substitutePlaceholders("Min: {{PI_FF_RESEARCHER_MIN}}", {});
...
describe("PI_FF_RESEARCHER_MAX placeholder", () => {
  beforeEach(resetSettingsState);
  afterEach(resetSettingsState);

  it("replaces with the numeric value of researcherMaxInstances", () => {
    setSetting("researcherMaxInstances", 5);
    const result = substitutePlaceholders("Spawn {{PI_FF_RESEARCHER_MAX}} researchers", {});
    expect(result).toBe("Spawn 5 researchers");
    expect(result).not.toContain("{{PI_FF_RESEARCHER_MAX}}");
  });

  it("replaces with default value (3) when setting not changed", () => {
    const result = substitutePlaceholders("Max: {{PI_FF_RESEARCHER_MAX}}", {});
  ...
  // which is settings-ui's responsibility, not feature-flow's. The substitution logic itself is
...
describe("all researcher placeholders together", () => {
  ...
  it("replaces all three placeholders in same text", () => {
    ...
    const result = substitutePlaceholders(
      "docs/research/{{PI_FF_FEATURE_SLUG}}/{{PI_FF_FEATURE_SLUG}}-design-initial-agent-1.md (min={{PI_FF_RESEARCHER_MIN}}, max={{PI_FF_RESEARCHER_MAX}})",
      { slug: "my-feature" },
    );
