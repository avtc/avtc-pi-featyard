...
describe("ff:resume command", () => {
  ...
  test("command is registered with the expected name and description", () => {
    // Still need a temp CWD: loading the extension runs init → ensureFfJunction(process.cwd()),
    // which must not touch the real repo's .ff.
    withTempCwd();
    const { fakePi, commands } = createCommandTrackingPi();
    workflowMonitorExtension(fakePi as unknown as ExtensionAPI);
    expect(commands.has("ff:resume")).toBe(true);
    expect(commands.get("ff:resume")?.description).toMatch(/continue|workflow|feature/i);
  });
  ...
  test("requires UI — notifies error when hasUI is false", async () => {
    ...
    const handler = commands.get("ff:resume")?.handler;
  ...
  test("shows selection dialog with features, Skip, and Manage — selecting first loads it", async () => {
    ...
    const state1 = createFeatureState("2026-05-01-alpha", "docs/ff/designs/2026-05-01-alpha-design.md");
    state1.updatedAt = "2026-05-01T00:00:00.000Z";
    state1.workflow.currentPhase = "plan";
    saveFeatureState(state1, null);

    const state2 = createFeatureState("2026-05-02-beta", "docs/ff/designs/2026-05-02-beta-design.md");
    ...
    expect(process.env.PI_FF_FEATURE).toBe("2026-05-02-beta");

    // syncEnvVars should have set the stage env var
    expect(process.env.PI_FF_STAGE).toBe("implement");
  ...
  test("Skip — returns without loading anything", async () => {
    ...
    const state = createFeatureState("2026-05-03-skip", "docs/ff/designs/2026-05-03-skip-design.md");
  ...
  test("Manage state files — mark completed, loops back, then load remaining", async () => {
    ...
    const state1 = createFeatureState("2026-05-04-to-complete", "docs/ff/designs/2026-05-04-to-complete-design.md");
    state1.updatedAt = "2026-05-04T00:00:00.000Z";
    saveFeatureState(state1, null);

    const state2 = createFeatureState("2026-05-05-keep", "docs/ff/designs/2026-05-05-keep-design.md");
  ...
  test("replaces currently active workflow with a different one", async () => {
    ...
    const stateA = createFeatureState("2026-05-06-active", "docs/ff/designs/2026-05-06-active-design.md");
    stateA.updatedAt = "2026-05-07T00:00:00.000Z";
    stateA.workflow.currentPhase = "implement";
    saveFeatureState(stateA, null);
    process.env.PI_FF_FEATURE = "2026-05-06-active";

    // Feature B — the one we want to switch to (sorted second)
    const stateB = createFeatureState("2026-05-07-switch", "docs/ff/designs/2026-05-07-switch-design.md");
    ...
    expect(process.env.PI_FF_FEATURE).toBe("2026-05-07-switch");
  ...
  test("Manage state files — delete action removes state file, loops back, then load remaining", async () => {
    ...
    const state1 = createFeatureState("2026-05-08-to-delete", "docs/ff/designs/2026-05-08-to-delete-design.md");
    state1.updatedAt = "2026-05-08T00:00:00.000Z";
    saveFeatureState(state1, null);

    const state2 = createFeatureState("2026-05-09-keep", "docs/ff/designs/2026-05-09-keep-design.md");
  ...
  test("Manage state files — cancel (null) loops back without mutating state", async () => {
    ...
    const state1 = createFeatureState("2026-05-10-persist", "docs/ff/designs/2026-05-10-persist-design.md");
  ...
  test("unrecognized select choice — exits loop without error or loading", async () => {
    ...
    const state = createFeatureState("2026-05-11-unrecognized", "docs/ff/designs/2026-05-11-unrecognized-design.md");
    saveFeatureState(state, null);
  ...
  test("select returns undefined (user cancels) — exits without loading or warning", async () => {
    ...
    const state = createFeatureState("2026-05-12-cancel", "docs/ff/designs/2026-05-12-cancel-design.md");
    saveFeatureState(state, null);
  ...
  test("switches to last session when feature has sessionFiles", async () => {
    ...
    const state = createFeatureState(
      ...
      "docs/ff/designs/2026-05-13-session-switch-design.md",
    );
    ...
    expect(process.env.PI_FF_FEATURE).toBe("2026-05-13-session-switch");
