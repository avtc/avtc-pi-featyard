...
describe("ff:reset command", () => {
  ...
  test("command is registered with the expected name and description", () => {
    ...
    workflowMonitorExtension(fakePi as unknown as ExtensionAPI);
    expect(descriptions.has("ff:reset")).toBe(true);
    expect(descriptions.get("ff:reset")).toMatch(/reset/i);
  });
  ...
  test("resets persisted state — workflow, tdd, debug, verification all return to defaults", async () => {
    ...
    // Call the command
    const handler = commands.get("ff:reset");
  ...
  test("clears PI_FF_FEATURE env var and activeFeatureSlug", async () => {
    process.env.PI_FF_FEATURE = "2026-05-10-test-feature";
    const { commands } = setup();
  ...
  test("preserves state file on disk — does not delete it", async () => {
    try {
      ...
      const state = createFeatureState(slug, "docs/ff/designs/2026-05-10-persist-test-design.md");
