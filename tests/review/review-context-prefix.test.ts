...
// resolves {{PI_FF_REVIEW_LOOP_CONTEXT}}, {{PI_FF_REVIEW_METHOD}}, and
// {{PI_FF_DESIGN_DOC_PATH}}/{{PI_FF_PLAN_DOC_PATH}}. They are exercised via integration in
...
// manual /skill:ff-design-review|plan-review invocation).
//
...
describe("review context helper prerequisites", () => {
  test("design-review skill file exists (required by computeReviewMethod)", () => {
    expect(fs.existsSync("skills/ff-design-review/SKILL.md")).toBe(true);
  });

  test("plan-review skill file exists (required by computeReviewMethod)", () => {
    expect(fs.existsSync("skills/ff-plan-review/SKILL.md")).toBe(true);
  });
});
