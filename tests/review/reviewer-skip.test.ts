...
describe("reviewerEmptyLoops tracking", () => {
  ...
  test("increments empty loop count per reviewer", () => {
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    expect(_getEmptyLoopsForSlug("feature-1")).toEqual({ "fy-quality-reviewer": 1 });
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    expect(_getEmptyLoopsForSlug("feature-1")).toEqual({ "fy-quality-reviewer": 2 });
  });

  test("tracks different reviewers independently", () => {
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    _incrementEmptyLoop("feature-1", "fy-security-reviewer");
    expect(_getEmptyLoopsForSlug("feature-1")).toEqual({ "fy-quality-reviewer": 1, "fy-security-reviewer": 1 });
  });

  test("tracks different features independently", () => {
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    _incrementEmptyLoop("feature-2", "fy-quality-reviewer");
    expect(_getEmptyLoopsForSlug("feature-1")).toEqual({ "fy-quality-reviewer": 1 });
    expect(_getEmptyLoopsForSlug("feature-2")).toEqual({ "fy-quality-reviewer": 1 });
  });

  test("resets empty loop count for specific reviewer", () => {
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    _resetEmptyLoop("feature-1", "fy-quality-reviewer");
    expect(_getEmptyLoopsForSlug("feature-1")).toEqual({});
  });

  test("resetAllEmptyLoops clears everything", () => {
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    _incrementEmptyLoop("feature-2", "fy-security-reviewer");
    _resetAllEmptyLoops();
    expect(_getEmptyLoopsForSlug("feature-1")).toEqual({});
  ...
  test("isReviewerSkipped returns false when below threshold", () => {
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    expect(_isReviewerSkipped("feature-1", "fy-quality-reviewer", 2)).toBe(false);
  });

  test("isReviewerSkipped returns true when at threshold", () => {
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    expect(_isReviewerSkipped("feature-1", "fy-quality-reviewer", 2)).toBe(true);
  });
  ...
  test("isReviewerSkipped returns false when threshold is 0 (disabled)", () => {
    ...
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    expect(_isReviewerSkipped("feature-1", "fy-quality-reviewer", 0)).toBe(false);
  });

  test("isReviewerSkipped with threshold 1: skips after single empty loop", () => {
    _incrementEmptyLoop("feature-1", "fy-quality-reviewer");
    expect(_isReviewerSkipped("feature-1", "fy-quality-reviewer", 1)).toBe(true);
    expect(_isReviewerSkipped("feature-1", "fy-quality-reviewer", 2)).toBe(false);
  });
});
