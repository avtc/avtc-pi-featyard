...
beforeEach(() => {
  // The design-doc path is mode-dependent (committed → docs/featyard/designs, local → .featyard/designs);
...
describe("kanban skill substitution integration", () => {
  it("expandSkillCommand with substituteFn replaces design doc path placeholder", () => {
    ...
    const result = expandSkillCommand("/skill:fy-design Work on feature: test", substituteFn);
    // The expanded skill should have the exact path substituted
    expect(result).toContain("docs/featyard/designs/2026-05-22-my-feature-design.md");
  ...
  it("expandSkillCommand with substituteFn replaces plan doc path placeholder", () => {
    const slug = "2026-05-22-my-feature";
    const substituteFn = (text: string) => substitutePlaceholders(text, { slug });
    const result = expandSkillCommand("/skill:fy-plan Work on feature: test", substituteFn);
    // The expanded skill should have the exact path substituted
    expect(result).toContain(".featyard/task-plans/2026-05-22-my-feature-task-plan.md");
  ...
  it("expandSkillCommand without substituteFn leaves placeholders unsubstituted", () => {
    const result = expandSkillCommand("/skill:fy-design Work on feature: test", null);
    ...
    expect(result).toContain("{{PI_FY_DESIGN_DOC_PATH}}");
  });
});
